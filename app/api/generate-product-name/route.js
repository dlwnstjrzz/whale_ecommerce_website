...
상품명 구성 규칙:
   - 제품명은 핵심메인키워드, 메인키워드, 제품 특징의 조합으로 구성됨.
원본 상품명과 이미지 분석 제품 특징, 연관키워드에서 상품과 밀접하게 연관된 핵심 키워드들을 우선적으로 선별해 조합해줘.
단, 원본 상품명과 똑같으면 안 좋으니 핵심 메인 키워드를 제외한 나머지 핵심 키워드들은 원본 상품명에 없는 걸로 해줘.
...
제품 이미지 분석은 가능하지만, 이미지로부터 유추된 특징은 상품명에 직접적으로 아주 밀접한 연관이 있을 때만 반영해줘.
  예: 재질, 크기, 색상 등은 이미지에서 매우 눈에 띄거나 제품명이 이에 의존해야 할 때만 사용
      ...
      messages: [
        {
          role: "system",
          content:
            "당신은 이커머스 소매업 웹사이트(네이버 스마트스토어, 11번가, 지마켓 등의 오픈마켓)에 등록할 제품명을 생성하는 전문가입니다.",
        },
        {
          role: "user",
          content: prompt,
